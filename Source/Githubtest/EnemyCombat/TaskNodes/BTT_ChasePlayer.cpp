...
//     FAIMoveRequest MoveRequest{ PlayerLocation }; // This initializes a new FAIMoveRequest object with the destination set to PlayerLocation
//     MoveRequest.SetUsePathfinding(true);
//     MoveRequest.SetAcceptanceRadius(100.0f); //  How close enemy will move in range of the enemy
//
...
// Helper function to initialize and validate references.
bool UBTT_ChasePlayer::InitializeReferences(UBehaviorTreeComponent& OwnerComp)
{
    CharacterRef = Cast<ACharacter>(OwnerComp.GetAIOwner()->GetPawn());
    ControllerRef = OwnerComp.GetAIOwner();
    CachedOwnerComp = &OwnerComp;

    return CharacterRef && ControllerRef && CachedOwnerComp;
}

// Helper function to get the player’s location.
FVector UBTT_ChasePlayer::GetPlayerLocation()
{
    return GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation();
}

// Helper function to check if the task and necessary references are valid.
bool UBTT_ChasePlayer::IsTaskValid()
{
    return GetWorld()->GetFirstPlayerController()->GetPawn() && CharacterRef;
}